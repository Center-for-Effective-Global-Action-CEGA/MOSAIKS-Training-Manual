...
**<ins>MOSAIKS</ins>: <ins>M</ins>**ulti-task **<ins>O</ins>**bservation using **<ins>SA</ins>**tellite **<ins>I</ins>**magery & **<ins>K</ins>**itchen **<ins>S</ins>**inks 
:::

## The challenge

Right now, [numerous satellite systems](https://www.ucsusa.org/resources/satellite-database#.VF_jIlPF8Wg) collect huge amounts of data about the world every day. But there is so much imagery (terabytes per day) that it’s overwhelming to sort through by hand; and it’s too complex and unstructured to be usable in its raw form for most applications.

![Visual representation of trackable objects in orbit around Earth ([ESA 2008](https://www.esa.int/var/esa/storage/images/esa_multimedia/images/2008/03/trackable_objects_in_orbit_around_earth/10152594-2-eng-GB/Trackable_objects_in_orbit_around_Earth_pillars.jpg)).](../../images/every-single-satellite-1.jpg){#fig-satellites width=90%}

That is why linking satellite imaging to machine learning (sometimes referred to as SIML, SatML, or GeoML) is incredibly powerful. It enables vast amounts of unstructured image data to be transformed into structured information that can be used for planning, research, and decision-making.  
...
<!-- ![Traditional framework of deep learning models for satellite image processing. ([Kumar et al. 2021](https://onlinelibrary.wiley.com/doi/full/10.1002/ett.3988))](https://www.researchgate.net/publication/341784517/figure/fig3/AS:1095936138842113@1638302945367/Deep-learning-models-for-satellite-image-processing.png){#fig-satellite-deep-learning} -->

![Traditional framework of deep learning models (i.e., machine learning with artificial neural networks) applied to imagery. In this example, the model is attempting to classify what vehicle the image contains [@al-khowarizmi_etal_2023{.citation}].](../../images/CNN.jpg){#fig-cnn-deep-learning}
...
- [The seminal MOSAIKS paper (Rolf et al. 2021)](https://www.nature.com/articles/s41467-021-24638-z)

- [A 2 page high-level overview of the MOSAIKS framework](../../slides/MOSAIKS2Pager_Int.pdf)
:::
...
In many cases, users aiming to predict an outcome from satellite imagery can do so using pre-computed imagery features (**X**) in a simple linear regression framework. Later in this training course, we will detail more customized workflows that remain tractable but allow for more flexibility. In the standard case, however, the procedure for using MOSAIKS has five steps (@fig-mosaiks-overview):

1. [Download pre-computed MOSAIKS features](https://www.mosaiks.org/mosaiks_api) (**X**) corresponding to the locations where you have labels. In @fig-mosaiks-overview, this corresponds to the matrix in the center of the top row.

1. Merge the features with your labels (**Y**) based on location (so features at position **P** are linked to labels at position **P**).

1. Run a cross-validated ridge regression of your labels on the MOSAIKS features (**Y = Xβ + e**; or any other model you choose!) 

1. Evaluate performance on a held out test set.
...
The figure below is from the seminal MOSAIKS publication (Rolf et al. 2021). The left maps show the input labels. The right map shows the modeled predictions. The scatter plot shows the modeled predictions against the true labels and reports the coefficient of determination (R²) as a measure of performance. 
...
The world of SIML is vast and rapidly evolving. There is a good choice you do not have to make global scale predictions yourself. Instead, you might be able to use or build off the hard work of many others in the field.
...
:::{.callout-tip}
Another great option is Google's Alpha Earth Foundation model Satellite Embeddings dataset, which provides pre-computed features at 10 m resolution for the entire globe. There is one global snapshot per year going back to 2017. These features can be treated very similarly to MOSAIKS features. You can access the dataset on [Google Earth Engine](https://developers.google.com/earth-engine/datasets/catalog/GOOGLE_SATELLITE_EMBEDDING_V1_ANNUAL).
:::

## Lecture materials

To assist with teaching this material, we have prepared lecture slides that cover the key concepts of @rolf_etal_2021. These slides are intended to provide a visual overview of the MOSAIKS framework, its capabilities, and its applications. They can be used as a starting point for lectures, presentations, or self-study on the topic of MOSAIKS and satellite imagery analysis.

These slides can be found [here](../../slides/00-intro-lecture.pdf) and are also included in the GitHub repository for this training manual.
