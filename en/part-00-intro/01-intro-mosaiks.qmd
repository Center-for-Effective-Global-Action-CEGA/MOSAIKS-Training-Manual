...
This process is illustrated by @fig-mosaiks-overview, which is from the original MOSAIKS publication (Rolf et al. 2021). Panels A and C shows the imagery input and transformation which is not needed by most users. Instead, users interact primarily with the tabular features and labels to run simple regressions (the right side of panel B). 
...
![Income input data (left) from the American Community Survey (ACS) 5-year estimates of median annual household income in 2015. These data are accessible using the [acs package](https://cran.r-project.org/web/packages/acs/acs.pdf) in R, table number B19013](../../images/f2_income.jpg){#fig-income}
