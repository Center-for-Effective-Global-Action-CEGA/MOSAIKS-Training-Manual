...
::: {.callout-note}
MOSAIKS grid cells begin at [null island](https://maps.app.goo.gl/bSHXKLEPhPLSrjgv6). The data you receive from the API has grid cell centroids as the longitude and latitude columns that are rounded to 0.005 degrees and are spaced by 0.01 degree (e.g., 10.005, 10.015, 10.025,...).
:::
...
Continuous variables take numeric values along a spectrum, such as crop yields (e.g., tons per hectare) or population density (people per square kilometer).
...
Categorical variables group observations into distinct classes or categories. These can be binary categories such as presence/absence of mining, or they can have multiple classes such as land use types (urban/agriculture/forest), or crop types (maize/wheat/rice). Labels will be prepared essentially the same way regardless of the number of categories, but the modeling approach and evaluation metrics will differ.

1. **Binary classification**: For variables with two categories (e.g., presence/absence), users should choose a model that can output probability predictions between 0 and 1. In these cases, a good choice of model would be a logistic regression or a decision tree. Performance is typically evaluated using metrics like accuracy, precision, recall, or area under the receiver operating characteristic curve (AUC-ROC).

2. **Multi-class classification**: For variables with multiple categories, users should choose a model that can handle multi-class classification, such as a random forest or a support vector machine. Performance is typically evaluated using metrics like accuracy, precision, recall, or F1-score.
...
This section only provides a brief overview of the two types of variables and some of the metrics used to evaluate them. This will be covered in greater detail in @sec-model-choice.
:::

## Joining Data

Merging Labels with MOSAIKS Features is an essential step in preparing your dataset for modeling. This process involves aligning the spatial and temporal dimensions of your label data with the MOSAIKS features. The goal is to create a unified dataset where each observation includes both the label (the variable you want to predict) and the corresponding features (the satellite-derived data).

To merge labels with features, you must align the geographic location of both datasets. The native resolution MOSAIKS features ($0.01\degree$) have rows for each location and columns with longitude, latitude, and 4,000 features. The lat/lon information represents the centroid of each grid cell. Your label dataset must also have columns for longitude, latitude, and label values (at minimum). Alternatively, aggregated features will come with the name of the administrative region (e.g., district) and the features. You can join this with your label data by matching the district names.
...
| Observation | District  | Year | Ft. 1 | Ft. 2 | ... | Ft. 4,000 |
|-------------|-----------|------|-----------|-----------|-----|-------------|
...
| Observation | District  | Year | Crop Yield | Ft. 1 | Ft. 2 | ... | Ft. 4,000 |
|-------------|-----------|------|------------|-----------|-----------|-----|-------------|
...
In the above example, our geographic location is the district, our time period is 2019, and our label is the crop yield (mt/ha). We can use the District and Year columns to join the label data with the MOSAIKS features. We then have 4,000 columns containing the features paired to *N* observations that are ready to be modeled. 

## Data cleaning considerations

### Coordinate reference systems (CRS)

The default coordinate reference system used by MOSAIKS is World Geodetic System 1984 (WGS84). WGS84 is a [Geographic Coordinate System (GCS)](https://resources.arcgis.com/en/help/main/10.1/index.html#//003r00000006000000). The standard that defines WGS 84 is ["EPSG:4326"](https://epsg.io/4326). EPSG stands for the [European Petroleum Survey Group](https://en.wikipedia.org/wiki/EPSG_Geodetic_Parameter_Dataset), which maintains a database of coordinate systems and projections. The WGS 84 coordinate system is the most commonly used coordinate system for GPS data. 

Alternatively, you may have data in a [projected coordinate system](https://resources.arcgis.com/en/help/main/10.1/index.html#//003r00000006000000) (e.g., UTM, Mercator, etc.). Projected coordinate systems are based on a map projection, which is a mathematical transformation of the Earth's surface onto a flat plane. There are many different map projections, each with its own advantages and disadvantages. It is usually best practice to reproject vector data (points, lines, polygons) to WGS 84 before joining with MOSAIKS features. 

![Seven map projections showing how 2 dimensional visualizations of 3 dimensional objects results in at least one of four basic characteristics (distance, direction, shape, and area) becoming distorted. Source: [geoawesome.com](https://geoawesome.com/map-distortions/)](../../images/projections.jpg){#fig-map-distortions}
...
::: {.callout-note}
# Learn more about CRS and map projections

Another excellent resource for learning about coordinate reference systems and map projections is the book titled[Spatial Data Visualization with tmap](https://tmap.geocompx.org/map-projections) [@nowosad_2026]. Map projections can be visualized and compared to one another [here](https://map-projections.net/imglist.php). 
:::
...
Currently the only publicly available MOSAIKS features are from a single global composite of satellite imagery from 2019. If your labels are from a different time period, you will need to ensure that they are either slow to change (e.g., relative wealth) or that they are from a time period close to 2019 and that the features you use are relevant to that time period. 

If you have labels that require imagery from the same time period (and that is not 2019) or you have time series labels, you will need to compute custom features for the relevant time period(s). See @sec-features-computing for more information on how to do this and @sec-model-time for guidance on modeling time series data with MOSAIKS features.
...
- [ ] **Consistently geolocated** as point, line, polygon, raster, or administrative data
