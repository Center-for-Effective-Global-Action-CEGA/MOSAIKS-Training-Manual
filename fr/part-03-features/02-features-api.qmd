...
# Caractéristiques API {#sec-features-api}
...
Dans ce chapitre, nous nous concentrons sur les **caractéristiques MOSAIKS disponibles via l’API** ([MOSAIKS API](https://api.mosaiks.org/)). Ces caractéristiques permettent d’intégrer rapidement et facilement des prédicteurs basés sur l’imagerie satellite dans vos analyses sans avoir à traiter manuellement les images satellites.

::: {.callout-note}
L’accès aux caractéristiques via l’API est expliqué en détail dans @sec-intro-api. Ce chapitre fournit des informations complémentaires sur la génération des caractéristiques et leurs spécifications. Si vous souhaitez générer vos propres caractéristiques, consultez @sec-features-computing.
:::
...
## Détails sur la génération des caractéristiques API

L’équipe MOSAIKS a effectué une **unique passe de featurization** sur l’imagerie Planet Labs 2019 Q3. Ce processus repose sur la méthode des **Caractéristiques de Convolution Aléatoires (RCFs)** décrite en @sec-features-rcf, avec les paramètres spécifiques suivants :
...
2. **Tailles de filtres (kernels)**  
   - **2 000 caractéristiques** dérivées de filtres 4×4.  
   - **2 000 caractéristiques** dérivées de filtres 6×6.  
...
5. **Vecteur final de caractéristiques**  
   - Après application de tous les filtres, chaque cellule 0.01° reçoit un **vecteur de 4 000 dimensions**.  
   - Ce processus a été exécuté **une seule fois**, générant une couche de caractéristiques globale fixe.

![Trois cartes d’activation de caractéristiques aléatoires extraites de l’imagerie Planet Labs.](../../images/RCF_patch_sampling_activation_maps_world.png){#fig-rcf-world-simple}

---

## Agrégations dérivées

Bien que les caractéristiques soient initialement calculées à **0.01° de résolution (~1 km²)**, l’API propose également des versions **agrégées** :
...
: Options de téléchargement des caractéristiques via l’API MOSAIKS. {#tbl-api-options}

Illustration des agrégations :

![Exemple de caractéristiques convolutionnelles aléatoires téléchargées à différentes échelles : (A) grille 0.1°, (B) départements, (C) régions.](../../images/US_feature_summaries.png){#fig-feature-aggregation}
...
## Quand utiliser ces caractéristiques ?

1. **Disponibilité immédiate** :  
   - Si vos labels datent de ~2019 (ou d’une période proche sans changements majeurs), **ces caractéristiques sont la solution la plus rapide et facile**.

2. **Données agrégées** :  
   - Si vos données sont agrégées (ex. niveau pays), téléchargez **directement les caractéristiques agrégées**.
...
2. **Processus de featurization unique**  
   - **Caractéristiques générées une seule fois** (pas d’actualisation périodique)  
   - L’agrégation (0.1°, 1°, ADM) repose sur les **mêmes caractéristiques 0.01°**  

3. **Caractéristiques RCF (Random Convolutional Features)**  
   - **4 000 dimensions**  
...
Dans la prochaine section, nous explorerons les techniques avancées pour **calculer et personnaliser vos propres caractéristiques MOSAIKS**.
:::
